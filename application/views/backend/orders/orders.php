<div class="bread_parent">
   <div class="col-md-12">
      <ul class="breadcrumb">
         ...
         <li><b>List Of Shipped Orders</b> </li>
      </ul>
   </div>
   <!-- <div class="col-md-5">
      ...
      <table id="datatable_example" class="table-bordered responsive table table-striped table-hover">
         ...
         <tbody>
            ...
            <tr>
                  ...
                  <?php
                      ...
                      if($gross_amount){
                        if($order_info->currency_type==1){
                          $totalgross_amount = $gross_amount * $order_info->currency_amount_in_ethereum;
                        }else if($order_info->currency_type==2){
                          $totalgross_amount = $gross_amount * $order_info->currency_amount_in_bitcoin;
                        }else{
                          $totalgross_amount = $gross_amount * $order_info->currency_amount_in_dollor;
                        }
                        echo getCurrencyIcon($order_info->currency_type).''.number_format($totalgross_amount, 8); 
                      }else{
                        echo "0.00";
                      }  
                  ?>
               ...
               <td>
                <?php
                    $totalShippingCharges = $row->totalShippingCharges;
                    if($totalShippingCharges){
                      if($order_info->currency_type==1){
                        $totaltotalShippingCharges = $totalShippingCharges * $order_info->currency_amount_in_ethereum;
                      }else if($order_info->currency_type==2){
                        $totaltotalShippingCharges = $totalShippingCharges * $order_info->currency_amount_in_bitcoin;
                      }else{
                        $totaltotalShippingCharges = $totalShippingCharges * $order_info->currency_amount_in_dollor;
                      }
                      echo getCurrencyIcon($order_info->currency_type).''.number_format($totaltotalShippingCharges, 8); 
                    }else{
                      echo "-";
                    }  
                ?>
               ...
               <td>
                <?php
                    ...
                    if($total_amount){
                      if($order_info->currency_type==1){
                        $totaltotal_amount = $total_amount * $order_info->currency_amount_in_ethereum;
                      }else if($order_info->currency_type==2){
                        $totaltotal_amount = $total_amount * $order_info->currency_amount_in_bitcoin;
                      }else{
                        $totaltotal_amount = $total_amount * $order_info->currency_amount_in_dollor;
                      }
                      echo getCurrencyIcon($order_info->currency_type).''.number_format($totaltotal_amount, 8); 
                    }else{
                      echo "0.00";
                    }  
                ?>
               ...
               <td>
                ...
                ?>
                  <a href="<?php echo base_url('backend/orders/order_details/'.$o_id.'/'.$orderDetailIDs); ?>" class="btn btn-primary btn-xs tooltips" rel="tooltip" data-placement="top" data-original-title=" View Order Details"><i class="fa fa-eye"></i>
                  </a>
