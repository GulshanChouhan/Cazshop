...
class Products extends CI_Controller
{
  ...
  public function index($offset=0){
    ...
    if(!empty($_GET))
    {
      ...
      $search[]=' pi.category_id like "%'.trim($_GET['category_id']).'%"';
      if(!empty($_GET['product_type']))
      $search[]=' pv.type_of_product = "'.trim($_GET['product_type']).'"';
      if(!empty($_GET['status']))
      $search[]=' pv.status = "'.trim($_GET['status']).'"';
    }
  ...
  public function product_category(){
    ...
    if(isset($_POST['selectProductCat'])){
      $productCat = $_POST['productcategories'];
  ...
  public function product_basic_info(){
    ...
    $data['title']='Add Product Vital Info';
 ...
 $data['category'] =$this->common_model->get_brands_category('category',array('category_id','category_name','parent_id'),$sessionSubCat1);
   ...
   $data['attribute_info'] = $attribute_info = $this->common_model->get_result_using_findInSet('attributes', array('type' => 1, 'status' => 1),'','','',array($sessionSubCat,'category_id'),'attribute_code');
    ...
    $data['brand_info'] = $brand_info = $this->common_model->get_brands('brand', array($sessionSubCat,'category_id'), seller_id());
  ...
  public function edit_product_basic_info($product_info_id='', $product_variation_id='', $type=''){
    ...
    $data['title']='Edit Product Vital Info';
    ...
    $data['brand_category'] =$this->common_model->get_brands_category('category',array('category_id','category_name','parent_id'),$sessionSubCat1);
  ...
  public function product_variations($product_info_id='', $product_variation_id='', $type=''){
    ...
    $data['variation_themes'] = $variation_themes = $this->common_model->get_result('variation_themes', array('category'=>$category_id, 'status' => 1),"", array('product_theme_title','ASC'));
  ...
  public function product_offer($product_info_id='', $product_variation_id='', $type=''){
    ...
    $this->form_validation->set_rules('warranty_description', 'Warranty Description', 'trim'); 
    $this->form_validation->set_rules('seller_description', 'Seller Description', 'trim'); 
    $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
      if ($this->form_validation->run() == TRUE){ 
        $seller_id = seller_id();
        $insert = [
  ...
  public function product_other_info($product_info_id='', $product_variation_id='', $type=''){
    ...
    if (isset($_POST['submitProductOtherInfo'])){ 
      ...
      if ($id = $this->common_model->update('products_info', $updateData, array('product_info_id' => $product_info_id))){ 
        ...
        $this->session->set_flashdata('msg_success', 'Product Other Information '.$msgContent.' successfully');
        $redirectionResult = $this->common_model->get_row('product_variations', array('product_info_id' => $product_info_id,'product_variation_id' => $product_variation_id)); 
  ...
  public function product_images($product_info_id='', $product_variation_id='', $type=''){
    ...
    public function product_video($product_info_id='', $product_variation_id='', $type=''){

    $this->_check_login(); //check login authentication
    $this->_check_stepForm(); //check registration step authentication
    
    $this->form_validation->set_rules('video_link', 'Video Link', 'required|trim');
    $this->form_validation->set_rules('mirror_image', 'Product Image', 'callback_productmirrorImg_video');
    $this->form_validation->set_error_delimiters('<div class="error">', '</div>');

    if ($this->form_validation->run() == TRUE){
      if (!empty($this->session->userdata('productImg_check')['image'])) {

            $mirror_image=$this->session->userdata('productImg_check');
           $insert = [
        ...
        if (!empty($insertID)){ 
          $this->session->unset_userdata('productImg_check');
          $this->session->set_flashdata('msg_success', 'Product Mirror Images added successfully');
  ...
  public function product_descriptions($product_info_id='', $product_variation_id='', $type=''){
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if ($id = $this->common_model->update('products_info', $insert, array('product_info_id' => $product_info_id))){ 
        ...
        $this->session->set_flashdata('msg_success', 'Product Description '.$msgContent.' successfully'); 
  ...
  public function product_keywords($product_info_id='', $product_variation_id='', $type=''){
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if ($id = $this->common_model->update('products_info', $insert, array('product_info_id' => $product_info_id))){
        ...
        if(!empty($product_info->keywords)) 
          $msgContent = "updated"; 
        else 
          $msgContent = "added";

        $this->session->set_flashdata('msg_success', 'Product Keywords '.$msgContent.' successfully');
  ...
  public function product_seo($product_info_id='', $product_variation_id='', $type=''){
    ...
    $this->form_validation->set_rules('meta_title', 'Meta Title', 'max_length[60]|trim'); 
    $this->form_validation->set_rules('meta_keywords', 'Meta Keywords', 'max_length[100]|trim');
    $this->form_validation->set_rules('meta_description', 'Meta Description', 'max_length[170]|trim'); 
    $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if(empty($product_seo_info)){
        ...
        if ($id = $this->common_model->insert('product_seo', $insert)){
          ...
          $seller_info = $this->common_model->get_row('users', array('user_id' => seller_id()));

          //-------------For Send email AND SMS-----------
          $productTypeName = ($type==1) ? "Single Product" : "Variation Product"; 
          ...
          if(!empty($email_template)){
              ...
              $param     = array(
                  'site_name'           => SITE_NAME,
                  'seller_name'         => selleradmin_name(),
                  'product_title'       => ucfirst($product_info->title),
                  'product_type'        => $productTypeName,
                  'product_detail_link' => '<a target="_blank" href="'.base_url('seller/products/edit_product_basic_info/'.$product_info_id.'/'.$product_variation_id.'/'.$type).'">Click here to view/edit the details of this product</a>',
              ...
              $param     = array(
                  ...
                  'product_title'       => ucfirst($product_info->title),
                  'product_type'        => $productTypeName,
                  'product_detail_link' => '<a target="_blank" href="'.base_url('backend/products/edit_product_category/'.$product_info_id.'/'.$product_variation_id.'/'.$type).'">Click here to view the details of this product</a>',
                  'support_link'        => '<a target="_blank" href="'.base_url('seller/support/messages').'">Support</a>'
              );
  ...
  public function shipment_rate($product_info_id='', $product_variation_id=''){
    ...
    $data['title']='Shipment rates of Product';
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if ($resultUpdate){ 
        $this->session->set_flashdata('msg_success', 'Product shipment rate information updated successfully'); 
        if(!empty($product_info) && !empty($product_variations) && !empty($product_img)){
            redirect('seller/products/index');
        }else{
            redirect('seller/products/mydraft');
  ...
  public function delete_product($product_info_id='', $product_variation_id='', $type=''){
    ...
    function add_brand(){
     ...
     $brand_name=$this->common_model->get_row('brand', array('brand_name' =>$this->input->post('brand_name')));

       if(!empty($brand_name)){
         echo json_encode(array('status'=>'failed', 'msg'=>'brand-error'));
        ...
        if($this->common_model->insert('brand',$data_img)){
          ...
          $this->session->set_flashdata('msg_success','Brand information added successfully');
            ...
            echo json_encode(array('status'=>'failed', 'msg'=>'data not inserted. please try again'));
        $this->session->set_flashdata('msg_error','Sorry! Adding process has been failed. Please try again');
          ...
          if (($image[0] < 100 || $image[1] < 50) || ($image[0] > 300 || $image[1] > 100)) {
            echo json_encode(array('status'=>'failed', 'msg'=> 'Oops! Your brand image needs to be atleast of 100 X 50 pixels and at most of 300 X 100 pixels'));
          ...
          if (!$this->upload->do_upload('brand_image')){
            echo json_encode(array('status'=>'failed', 'msg'=>$this->upload->display_errors()));   
          ...
          echo json_encode(array('status'=>'failed', 'msg'=>'The %s field required'));   
      ...
      if(!$category) {
          ...
          if($data) {
              ...
              foreach ($data as $row) {
                  ...
                  if(!empty($dataCats)) $subcategoryExist = "<i class='icofont icofont-simple-right' aria-hidden='true'></i>";

                  $optionData .= "<li mainnew='".$mainnew."' catval='".$row->category_id."' class='choose-product-name getsubcatnew sub-cat-name'>".ucfirst($row->category_name).$subcategoryExist."</li>";
              }
      ...
      if(isset($_POST['pfi']) && !empty($_POST['pfi']) && $_POST['pfi']!='0'){
        ...
        if($reg){
          ...
          if($result1){
            ...
            $this->session->set_flashdata('msg_error', 'Sorry! Featured image not changed. please try again');
            redirect($_SERVER['HTTP_REFERER']); 
          }
        }else{
          $this->session->set_flashdata('msg_error', 'Sorry! Featured image not changed. please try again');
        ...
        $this->session->set_flashdata('msg_warning', 'This image is already your featured image');
        redirect($_SERVER['HTTP_REFERER']);
      }
    }
  }
  ...
  public function checkSellerSKU() {
    ...
    public function productmirrorImg_video() {
    $name_array = array();
    if(!empty($_FILES['mirror_image']['name'])){
              $config['encrypt_name']=true;
              $config['upload_path'] = './assets/uploads/seller/products/'; 
              $config['allowed_types'] = 'jpg|png|jpeg|gif'; 
              $config['max_size'] = '5024'; 
              ...
              $this->load->library('upload', $config); 
              $this->upload->initialize($config); 
              if (!$this->upload->do_upload('mirror_image')){ 

          
               $this->form_validation->set_message('productmirrorImg_video', $this->upload->display_errors()); 
                return FALSE; 
              }else{ 
                $data = $this->upload->data(); // upload image
                $config_img_p['source_path'] = './assets/uploads/seller/products/';
       ...
       if($name_array){
          $this->session->set_userdata('productmirrorImg_video', array('image' => $name_array)); 
          return TRUE;
        }
       }else{
        $this->form_validation->set_message('productmirrorImg_video', 'Please select a Video mirror image to upload'); 
        return FALSE; 
      }
    ...
    if(empty($product_info_id)){
      ...
      if($data){
        ...
        foreach ($data as $row) {
          ...
          $statustooltipFullMsg = ($row->status==2) ? "Do you want to enable this product for selling ?" : "Do you want to disable this product ?";
          ...
          if($pageType=='inventoryReport'){ 
            $pageAccess = '<td class="sellprice1 font-roboto text-center">$ '.$sellersell_price.'</td><td class="bestprice1 font-roboto text-center">$ '.$sellerbase_price.'</td><td class="quntity-no1 text-center">'.$row->quantity.'</td>';  
          }else if($pageType=='managePricing'){ 
            $pageAccess = '<td class="sellprice1 font-roboto text-center"><div class="input-group"><div class="input-group-addon">$</div><input class="form-control input-update-form" type="number" maxlength="7" min="0" max="99999" id="sellprice'.$row->product_variation_id.'" name="sellprice" placeholder="15.00" disabled="disabled" value="'.$sellersell_price.'"><a href="javascript:void(0)" class="tooltips updateSellPrice" attrid="'.$row->product_variation_id.'" rel="tooltip" data-placement="top" data-original-title="Click to Edit Sell Price"><i class="fa fa-pencil" aria-hidden="true" id="iconSellPrice'.$row->product_variation_id.'"></i></a></div></td><td class="bestprice1 font-roboto text-center"><div class="input-group"><div class="input-group-addon">$</div><input class="form-control input-update-form" type="number" maxlength="7" min="0" max="99999" id="base_price'.$row->product_variation_id.'" name="base_price" placeholder="10.00" disabled="disabled" value="'.$sellerbase_price.'"><a href="javascript:void(0)" class="tooltips updateBasePrice" attrid="'.$row->product_variation_id.'" rel="tooltip" data-placement="top" data-original-title="Click to Edit Base Price"><i class="fa fa-pencil" aria-hidden="true" id="iconBasePrice'.$row->product_variation_id.'"></i></a></div></td><td class="quntity-no1 text-center">'.$row->quantity.'</td>';  

          }else{ 
            $pageAccess = '<td class="sellprice1 font-roboto text-center">'.$sellersell_price.'</td><td class="bestprice1 font-roboto text-center">'.$sellerbase_price.'</td><td class="quntity-no1 text-center"><div class="update-form-quantity"><input class="form-control input-update-form" type="number" onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="3" min="0" max="999" id="quantity'.$row->product_variation_id.'" name="quantity" placeholder="01" disabled="disabled" class="quantityNo" value="'.$row->quantity.'"><a href="javascript:void(0)" class="tooltips updateQuantity" attrId="'.$row->product_variation_id.'" rel="tooltip" data-placement="top" data-original-title="Click to Edit Quantity">&nbsp;<i class="fa fa-pencil" aria-hidden="true" id="iconQuan'.$row->product_variation_id.'"></i></a></div></td>';  
          }
          $tempMore.= $adminApprovel.'<b>Created Date - </b><i class="fa fa-calendar-o" aria-hidden="true"></i> '.date('d M Y',strtotime($row->created_at));
          $resultdata .= '<tr class="child_'.$row->product_info_id.'"><td class="checkbox-status1">'.$s_no.'.'.$i.'</td><td class="productimg1 text-center">'.$img.'</td><td class="producttitle1">'.ucwords($row->title).' <b>('.$row->product_ID.')</b><br>'.$tempMore.'</td><td class="sku-no1 text-center">'.$sellerSKU.'</td>'.$pageAccess.'<td class="freeprive1 font-roboto text-center">'.$feePreview.'</td><td class="action1 text-center"><a onClick=\'javascript: return confirmBox("Do you want to change the status ?" , "'.base_url().'backend/common/change_status/product_variations/product_variation_id/'.$row->product_variation_id.'/'.$statusVal.'");\' class="btn btn-'.$statusCls.' btn-xs tooltips" rel="tooltip" data-placement="top" data-original-title="Click to '.$statustooltipMsg.'">'.$statusMsg.'</a>&nbsp;<a href="'.base_url().'seller/products/product_faq/'.$row->product_variation_id.'" class="btn btn-warning 
common-btn-xs btn-xs tooltips" rel="tooltip" data-placement="top" data-original-title="List of product FAQs"><i class="fa fa-question"></i></a>&nbsp;<a href="'.base_url().'seller/products/product_offer/'.$row->product_info_id.'/'.$row->product_variation_id.'/2" class="btn btn-primary 
common-btn-xs btn-xs tooltips" rel="tooltip" data-placement="top" data-original-title=" Edit Product" target="_blank"><i class="icon-pencil"></i></a>&nbsp;<a href="javascript:void(0)" onClick=\'javascript: return confirmBox("Do you want to delete it ?" , "'.base_url().'seller/products/delete_product/'.$row->product_info_id.'/'.$row->product_variation_id.'/2");\' class="btn btn-danger btn-xs common-btn-xs delete-btn-xs tooltips" rel="tooltip" data-placement="top" data-original-title="Delete Product"><i class="icon-trash"></i></a></td></tr>';
        }
  ...
  public function product_faq($product_variation_id='', $offset=0){
    ...
    if(!empty($_GET))
    {
      ...
      $search[]=' pf.question like "%'.trim($_GET['question']).'%"';
      if(!empty($_GET['answer']))
      $search[]=' pf.answer like "%'.trim($_GET['answer']).'%"';
    }
  ...
  public function add_product_FAQs($product_variation_id=''){
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if ($id = $this->common_model->insert('product_faq', $insert)){ 
        $this->session->set_flashdata('msg_success', 'Product FAQs information added successfully');
  ...
  public function edit_product_FAQs($product_variation_id='', $product_faq_id=''){
    ...
    if ($this->form_validation->run() == TRUE){ 
      ...
      if ($id = $this->common_model->update('product_faq', $insert, array('product_faq_id' => $product_faq_id))){ 
        $this->session->set_flashdata('msg_success', 'Product FAQs information updated successfully');
        redirect('seller/products/product_faq/'.$product_variation_id);
      }else{
        $this->session->set_flashdata('msg_error', 'Something went wrong! Please try again'); 
        redirect($_SERVER['HTTP_REFERER']); 
  ...
  public function managePricing($offset=0){
    ...
    $data['products'] = $this->seller_model->get_productresult($offset, PER_PAGE, $search);
    $config=backend_pagination();
    $config['base_url'] = base_url().'seller/products/managePricing';
    $config['total_rows'] = $this->seller_model->get_productresult(0, 0, $search);    
    $config['per_page'] = PER_PAGE;
    $config['uri_segment'] = 4;
    if(!empty($_SERVER['QUERY_STRING']))
  ...
  public function featured_image_change(){



      $this->_check_login(); //check login authentication
      $this->_check_stepForm();
      $product_info_id = $this->uri->segment(4);
      $product_variation_id = $this->uri->segment(5);
      $product_img_id = $_POST['pfi'];
