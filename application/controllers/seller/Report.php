...
class Report extends CI_Controller
{
  ...
  public function orders($offset=0){
    ...
    if(!empty($_GET))
    {
      ...
      $search[]=' (od.order_status like "%'.trim($_GET['getstatus']).'%")';
      if(!empty($_GET['start_date']) && !empty($_GET['end_date']))
      $search[]='DATE_FORMAT(o.created, "%Y-%m-%d")>="'.trim($_GET['start_date']).'" and DATE_FORMAT(o.created, "%Y-%m-%d")<="'.trim($_GET['end_date']).'"'; 
    } 
    $data['orders'] = $this->common_model->get_orderInfo($offset, PER_PAGE, $search, "", seller_id(), 1);
    $config=backend_pagination();
    $config['base_url'] = base_url().'seller/report/orders';
    $config['total_rows'] = $this->common_model->get_orderInfo(0, 0, $search, "", seller_id(), 1);
    ...
    $data['template']='seller/report/orders';
    $data['offset']=$offset;
    $data['status']='';//$status;
  ...
  public function inventory($offset=0){
    ...
    if(!empty($_GET))
    {
      ...
      if(!empty($_GET['start_date']) && !empty($_GET['end_date']))
      $search[]='DATE_FORMAT(pi.created_at, "%Y-%m-%d")>="'.trim($_GET['start_date']).'" and DATE_FORMAT(pi.created_at, "%Y-%m-%d")<="'.trim($_GET['end_date']).'"'; 
      if(!empty($_GET['minquantity']) && !empty($_GET['maxquantity']) )
      $search[]='pv.quantity BETWEEN "'. trim($_GET['minquantity']). '" and "'. trim($_GET['maxquantity']).'"';
    }
    $data['products'] = $this->seller_model->get_productresult($offset, PER_PAGE, $search);
    $config=backend_pagination();
    $config['base_url'] = base_url().'seller/report/inventory/';
    ...
    $this->pagination->initialize($config);
    $data['pagination']=$this->pagination->create_links();
    $data['template']='seller/report/inventory';
    $data['offset']=$offset;
    $this->load->view('templates/seller/template',$data);
  }
  ...
  public function exportCSV($offset=0) {
    ...
    if($usersData = $this->common_model->get_orderInfo($offset, 10000000000, $search, "", seller_id(), 1, "export"))
    {  
      //p($usersData); die;
  ...
  public function inventoryExportCSV($offset=0) {
    ...
    if($usersData = $this->seller_model->get_productresult($offset, 10000000000, $search,"inventoryExport"))
    {  
