...
class Content extends CI_Controller
{
	...
	public function page_add($offset=0){
		$this->_check_login(); //check login authentication
		$data['title']='pages'; 
		...
		if ($this->form_validation->run() == TRUE){
			...
			if($this->superadmin_model->insert('pages',$page_data)){
				$this->session->set_flashdata('msg_success','Page information added successfully');
		...
		$data['template']='backend/page/page_add';
		$this->load->view('templates/superadmin_template',$data);
	}
	...
	public function page_edit($page_id='',$offset=''){
		...
		if ($this->form_validation->run() == TRUE){
			...
			if($this->superadmin_model->update('pages',$page_data,array('id'=>$page_id))){
		...
		$data['page'] = $this->superadmin_model->get_row('pages',array('id'=>$page_id));
		if(empty($data['page'])) redirect('backend/content/pages');
	...
	public function page_delete($page_id ='',$offset=''){
		...
		$data['title']='';
		if(empty($page_id)) redirect('backend/content/pages');
		if($this->superadmin_model->delete('pages',array('id'=>$page_id,'post_type'=>'page'))){
   ...
   public function client_feedback($offset=0)
    {
    	...
    	if(isset($_POST['update'])){
			...
			$this->superadmin_model->update('cms_client_feedback',array('order'=>$value),array('id'=>$key));
			}            
    ...
    public function client_feedback_delete($id='')
    {
        ...
        if($this->superadmin_model->delete('cms_client_feedback',array('id'=>$id))){
    ...
    public function sub_page_banner()
   	...
   	{   
	    ...
	    $this->form_validation->set_error_delimiters('<div class="error">', '</div>');
			...
			if($this->superadmin_model->update('sub_page_banners', $data_update,array('id'=>$id))){
	             if($this->session->userdata('banner_img_check')):
						$this->session->unset_userdata('banner_img_check');				   
					endif;
    ...
    public function banner_img_check($str){
		...
		$image = getimagesize($_FILES['banner_img']['tmp_name']);
	       if ($image[0] != 1600 || $image[1] != 200) {
	           $this->form_validation->set_message('banner_img_check', 'Oops! Your banner image needs to be exactly 1600 x 200 pixels');
	           return FALSE;
	       }
		if(!empty($_FILES['banner_img']['name'])):
   ...
   function check_bulk_it_img($str){
    ...
    if(!empty($_FILES['bulk_it_img']['name'])):
      ...
      if($_POST['type']==2){
	       if ($image[0] < 60|| $image[1] < 60) {
	           $this->form_validation->set_message('check_bulk_it_img', 'Oops! bulk it Slider image needs to be atleast 60 x 60 pixels');
	           return FALSE;
	       }
    ...
    public function open_store()
   {
      ...
      $data['title']='open_store';  
       ...
       public function we_love()
   {
      $this->_check_login(); //check login authentication
      $data['title']='we_love';  
            $data['we_love'] = $this->superadmin_model->get_row('cms_home_page',array('id'=>3));       
         	$this->form_validation->set_rules('page_content','Content','required');         	
         	$this->form_validation->set_error_delimiters('<div class="error">', '</div>');
			if ($this->form_validation->run() == TRUE){			
               ...
               // $open_store_data['main_content'] = $this->input->post('page_content');
				$open_store_data['updated_content'] = $this->input->post('page_content');	
				$open_store_data['updated'] = date('Y-m-d,H:i:s A');			               
				if($this->superadmin_model->update('cms_home_page',$open_store_data,array('id'=>3))){				
				$this->session->set_flashdata('msg_success','Content updated successfully');
				redirect('backend/content/we_love');
			    }else{
				$this->session->set_flashdata('msg_error','Failed, Please try again');
				redirect('backend/content/we_love');
			    }
		...
		$data['template']='backend/content/we_love';
		$this->load->view('templates/superadmin_template',$data);
   }
   ...
   public function check_background_img($str)
   {
    ...
    public function menus($offset=0){
        ...
        if($this->input->post('action_type')==2){
         if(isset($_POST['update'])){
				...
				$order = $this->input->post('order');
				foreach ($order as $key => $value) { 
				$this->superadmin_model->update('menu',array('order'=>$value),array('id'=>$key));
				}            
				$this->session->set_flashdata('msg_success','Order sequence updated successfully');
	     ...
	     if($this->input->post('action_type')==1){
			if(isset($_POST['add'])){
				...
				$menu_data = array(
					...
					if($this->superadmin_model->insert('menu',$menu_data)){
		...
		$data['offset']=$offset;
		$data['parent_menu']=$this->common_model->get_result('menu',array('parent_id'=>0));
	...
	public function menu_edit($menu_id='',$offset=''){
		...
		if ($this->form_validation->run() == TRUE){
			...
			if($this->superadmin_model->update('menu',$menu_data,array('id'=>$menu_id))){
		...
		if(!$data['menu'] = $this->superadmin_model->get_row('menu',array('id'=>$menu_id))){
			redirect('backend/content/menus');
		}
		$data['pages'] = $this->superadmin_model->get_result('pages',array('status'=>1));
	...
	public function menu_delete($menu_id =''){
		$this->_check_login(); //check login authentication		
		if(empty($menu_id)) redirect('backend/content/menus');
		if($this->superadmin_model->delete('menu',array('id'=>$menu_id))){
