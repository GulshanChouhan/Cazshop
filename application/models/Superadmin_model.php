...
class Superadmin_model extends CI_Model {
	...
	public function getOrder_countAndTotal($user_id='', $days=''){
		$this->db->select('COUNT(o.o_id) AS total_orders,  SUM(od.subtotal) AS total_amounts, od.order_status, avg(o.currency_amount_in_bitcoin) as currency_amount_in_bitcoin, avg(o.currency_amount_in_ethereum) as currency_amount_in_ethereum, avg(o.currency_amount_in_dollor) as currency_amount_in_dollor');
		...
		if(!empty($days)){
			if($days!=1){
				$this->db->where('od.created >= DATE(NOW()) - INTERVAL '.$days.' DAY',null);
			}else{
				$this->db->where('od.created >= DATE(NOW()) - INTERVAL 0 DAY',null);	
			}
		}
		
		$this->db->where('od.order_status < ', 5, FALSE);
		$this->db->join('order_details as od', 'od.order_table_id = o.o_id');
	...
	public function getOrder_salesData($user_id='', $days=''){
		$this->db->select('UNIX_TIMESTAMP(od.created) as timestamp, od.created, count(od.order_detail_id) as totalorders, sum(od.subtotal) as subtotal, ROUND(sum(od.subtotal*od.commision_percentage/100), 2) as fee_preview');
		...
		if(!empty($days)){
			$this->db->where('od.created >= DATE(NOW()) - INTERVAL '.$days.' DAY');
		}

		$this->db->group_by('DATE_FORMAT(od.created, "%Y-%m-%d")');
		$this->db->where('od.order_status <', 5);
		$this->db->join('order_details as od', 'od.order_table_id = o.o_id');
		$this->db->from('orders as o');
		$query = $this->db->get();
		if($query->num_rows()>0)
